  ...
  <div class="home-content">
    <div class="content-boxes">
      <div class="recent-content box">
        <h3>修改會員</h3>
        <form action="/admin/updateMember/{{ $records['id'] }}" method="post">
            @csrf
            <div class="form-group">
              <label for="name">會員名稱</label>
              <input type="text" class="form-control" id="name" name="name" required="required" placeholder="請輸入會員名稱" value="{{ $records['name'] }}">
            </div>
            <div class="form-group">
              <label for="description">電子郵件</label>
              <textarea class="form-control" id="email" name="email" rows="3" required="required" placeholder="請輸入電子郵件">{{ $records['email'] }}</textarea>
            </div>
            <div class="form-group">
